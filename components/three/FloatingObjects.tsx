...
// ─── Product form (abstract bottle/container silhouette) ──────
function ProductForm({
  ...
  useFrame((state) => {
    ...
    ref.current.position.y =
      position[1] + Math.sin(t * speed) * 0.2 - scrollY.current * 1.2
    ref.current.position.x =
      position[0] + mouse.current.x * 0.15
  ...
  return (
    <group ref={ref} position={position} scale={scale}>
      {/* Body - rounded box as abstract container */}
...
// ─── Floating hex grid (manufacturing/tech feel) ──────────────
function HexGrid({
  position,
  mouse,
  scrollY,
...
export default function FloatingObjects({
  ...
  // Data stream connections between nearby nodes
  ...
  return (
    <group>
      {/* Network nodes — glass spheres with orbital rings */}
      {nodes.map((node, i) => (
        <NetworkNode
          key={`node-${i}`}
      ...
      {/* Data particle streams connecting nodes */}
